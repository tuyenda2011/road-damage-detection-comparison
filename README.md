...
như `yolo26n.pt`, `rtdetr-l.pt` hoặc pretrained Faster R-CNN; có thể tải trước và
...
```bash
python src/models/yolo/train.py --data configs/dataset.yaml --model yolo26n.pt --epochs 60 --imgsz 640 --batch 16 --device 0
```
...
Cấu hình mặc định dùng `yolo26n.pt` để phù hợp GPU 6 GB. Có thể đổi sang `yolo26s.pt`
nếu đủ VRAM và ưu tiên độ chính xác. Nên đặt weight tải sẵn trong `weights/` để thư mục
gốc không bị lộn xộn.
...
Giao diện dùng bố cục control-center: sidebar quản lý model/checkpoint/runtime; workbench chính
nhận ảnh hoặc video, hiển thị bounding box và cho phép so sánh ba model trên cùng ảnh. Hai khu
vực benchmark và visual analytics đọc trực tiếp `results/metrics.csv`. Trạng thái checkpoint và
thống kê split dữ liệu được hiển thị ngay trên dashboard.
