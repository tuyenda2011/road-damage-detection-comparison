...
```bash
python src/models/yolo/train.py --data configs/dataset.yaml --model yolov8n.pt --epochs 50 --imgsz 640 --batch 16 --device 0
```

Kết quả được lưu trong `runs/yolo/`, weight tốt nhất được copy về:
...
Mặc định config freeze 10 layer đầu để giảm VRAM/thời gian. Dùng `--freeze 0` để
fine-tune toàn bộ model. Dataset YAML được chuẩn hóa sang đường dẫn tuyệt đối trong
`runs/yolo/` nên lệnh train không phụ thuộc current working directory.

## Train Faster R-CNN

```bash
python src/models/faster_rcnn/train.py --config configs/faster_rcnn.yaml --epochs 30 --batch 4 --device cuda
```
...
inference gọn hơn; `last.pth` giữ optimizer/scheduler để phục vụ tiếp tục phát triển.

## Train RT-DETR

```bash
python src/models/rtdetr/train.py --data configs/dataset.yaml --model rtdetr-l.pt --epochs 50 --imgsz 640 --batch 8 --device 0
```

Kết quả được lưu trong `runs/rtdetr/`, weight tốt nhất được copy về:
...
```bash
python src/demo/demo_image.py --model yolo --weights runs/yolo/best.pt --source data/samples/test.jpg
```

Faster R-CNN:

```bash
python src/demo/demo_image.py --model faster_rcnn --weights runs/faster_rcnn/best.pth --source data/samples/test.jpg
```

RT-DETR:

```bash
python src/demo/demo_image.py --model rtdetr --weights runs/rtdetr/best.pt --source data/samples/test.jpg
```

Có thể chạy trực tiếp qua các file inference riêng:

```bash
python src/models/yolo/infer.py --weights runs/yolo/best.pt --source data/samples/test.jpg
python src/models/faster_rcnn/infer.py --weights runs/faster_rcnn/best.pth --source data/samples/test.jpg
python src/models/rtdetr/infer.py --weights runs/rtdetr/best.pt --source data/samples/test.jpg
```

## Demo video

```bash
python src/demo/demo_video.py --model yolo --weights runs/yolo/best.pt --source data/samples/road.mp4
```
...
```bash
python src/evaluation/evaluate.py --model yolo --weights runs/yolo/best.pt --data data/processed/test
```

Faster R-CNN:

```bash
python src/evaluation/evaluate.py --model faster_rcnn --weights runs/faster_rcnn/best.pth --data data/processed/test
```

RT-DETR:

```bash
python src/evaluation/evaluate.py --model rtdetr --weights runs/rtdetr/best.pt --data data/processed/test
```
