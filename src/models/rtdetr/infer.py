...
def load_rtdetr(weights: str | Path) -> RTDETR:
    weight_path = resolve_inference_weights(weights, "runs/rtdetr", ".pt")
...
def infer_rtdetr_image(
    ...
    model = load_rtdetr(weights)
    boxes, labels, scores = predict_rtdetr_image(model, image, conf=conf, device=device)
    result = draw_detections(image, boxes, labels, scores)
    out_path = ensure_dir(output) / f"{source_path.stem}_rtdetr_detected{source_path.suffix}"
    save_image(out_path, result)
    return out_path


def infer_rtdetr_video(
    weights: str,
    source: str,
    output: str = "results",
    conf: float = 0.25,
    ...
    try:
        ...
        out_path = ensure_dir(output) / f"{source_path.stem}_rtdetr_detected.mp4"
    ...
    with managed_video_output(cap, writer, out_path):
        ...
        while True:
            ...
            boxes, labels, scores = predict_rtdetr_image(model, frame, conf=conf, device=device)
...
def main() -> None:
    ...
    if source_kind(args.source) == "video":
        out = infer_rtdetr_video(args.weights, args.source, args.output, args.conf, args.device)
    else:
        out = infer_rtdetr_image(args.weights, args.source, args.output, args.conf, args.device)
